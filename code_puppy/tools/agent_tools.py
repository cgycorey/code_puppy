...
def run_agent_in_child_mode(agent_name: str, prompt: str) -> str:
...
def register_spawn_agent(agent):
    ...
    """
    @agent.tool
    def spawn_agent(context: RunContext, agent_name: str, prompt: str, background: bool = True, visible: bool = False) -> SpawnAgentOutput:
        """Spawn a sub-agent as a child process.
...
def register_check_running_agent_status(agent):
    ...
    """
    @agent.tool
    def check_running_agent_status(context: RunContext, agent_id: str) -> CheckRunningAgentStatusOutput:
        """Check the status of a running agent process.
    ...
    return check_running_agent_status
...
def register_list_running_agents(agent):
    ...
    """
    @agent.tool
    def list_running_agents(context: RunContext) -> ListRunningAgentsOutput:
        """List all currently running agents.
        ...
        try:
            ...
            if not agents:
                ...
                for agent_item in agents:
                    emit_system_message(
                        f"- [bold]{agent_item.agent_id}[/bold]: {agent_item.agent_name} (PID: {agent_item.pid}, Model: {agent_item.model or 'default'})", 
                        message_group=group_id
                    )
